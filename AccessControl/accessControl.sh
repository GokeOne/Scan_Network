	...
	declare -A active_users
 	...
 	tail -F "/var/log/mysql/mysql.log" 2>/dev/null | while read line; do
        	if echo "$line" | grep -q "Connect"; then
            		...
            		USER_IP=$(echo "$line" | awk '{print $4}')
			USER=$(echo "$USER_IP" | cut -d '@' -f 1)
            		IP=$(echo "$USER_IP" | cut -d '@' -f 2)
		        ...
		        if [ -n "$USER" ]; then
                		echo -e "\n[MySQL] User disconnected\nUser: $USER"
                		unset active_users["$SESSION_ID"]
            		else
                		echo -e "\n[MySQL] Uknown user disconnected (ID: $SESSION_ID)"
            		fi
        	fi
    	done
